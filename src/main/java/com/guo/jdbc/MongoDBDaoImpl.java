...
public class MongoDBDaoImpl {
    ...
    public MongoDBDaoImpl() {
        if (mongoClient == null) {
            ...
            MongoCredential credential = MongoCredential.createCredential(name, db_name, password.toCharArray());
            try {
                //数据库连接实例
                mongoClient = new MongoClient(new ServerAddress(ip, port), Collections.singletonList(credential), myOptions);
    ...
    public synchronized static MongoDBDaoImpl getInstance(String ip, int port) {
        if (mongoDBDaoImpl == null) {
            mongoDBDaoImpl = new MongoDBDaoImpl();
        }
